...
const handleMessageGPTStream = async (message: Message, prompt: string) => {
	try {
		// Get last conversation
		const lastConversationId = conversations[message.from];
		...
		// Check if we have a conversation with the user
		let conversationId: string = lastConversationId;
		if (!conversationId)
		{
			// Create new conversation
			const convId = randomUUID();
		...
		let _responseInterval: NodeJS.Timeout = setInterval(async () => {
			if (_response.length > 0) {
				if (!_reply) {
					_reply = await message.reply(_response); // reply with first message
				}
				else {
					await new Promise(resolve => setTimeout(resolve, Math.random() * 500)); // slow down the edit rate
					await _reply.edit(_response); // edit previous message with new data
				}
				cli.print(`[GPT] Stream to ${message.from}: ${_response}  | OpenAI request took ${Date.now() - start}ms)`);
			}
		}, 100);
		let response = await chatgpt.askStream(
			async (data: string) => {
				_response += data;
			},
			prompt, 
...
export { handleMessageGPT, handleMessageGPTStream, handleDeleteConversation };
